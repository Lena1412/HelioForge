...
def main() -> None:
    ...
    parser.add_argument("--json", type=str, default=None, help="Load SolarSystem from JSON export.")
    args = parser.parse_args()

    if args.json:
        from helioforge.export import load_json
        system = load_json(args.json)
        run(width=args.width, height=args.height, fps_limit=args.fps, mode=args.mode, system=system)
    else:
